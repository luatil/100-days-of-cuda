...
__global__ void RadixSortKernel(const unsigned int *XS, unsigned int *Output,
...
template <int BLOCK_DIM>
__global__ void ScatterValues(const unsigned int *XS, unsigned int *RadixSortResult, unsigned int *Output, int Bit,
                              int N)
{
    __shared__ unsigned int SharedNumberOfOnes, SharedNumberOfZeros;
    if (threadIdx.x == 0)
    {
        SharedNumberOfOnes = RadixSortResult[N];
        SharedNumberOfZeros = N - SharedNumberOfOnes;
    }
    __syncthreads();
    ...
    if (Tid < N)
    {
        unsigned int OnesBefore = RadixSortResult[Tid];
        unsigned int Target = ((XS[Tid] >> Bit) & 1) ? OnesBefore + SharedNumberOfZeros : Tid - OnesBefore;
        Output[Target] = XS[Tid];
    }
}

template <int BLOCK_DIM = 1024> static void RadixSort(const unsigned int *XS, unsigned int *Output, int N)
{
    dim3 BlockDim(BLOCK_DIM);
    dim3 GridDim(((N + 1) + BLOCK_DIM - 1) / BLOCK_DIM); // Have to compute number of zeros

    unsigned int *Flags, *ScanValue, *RadixSortResult;
    unsigned int *Input;
    ...
    CUDA_CHECK(cudaMalloc(&RadixSortResult, sizeof(unsigned int) * (N + 1))); // Have to compute number of zeros
    ...
    for (int Bit = 0; Bit < 32; Bit++)
    {
        ...
        CUDA_CHECK(cudaMemsetAsync(Flags, 0, sizeof(unsigned int) * (GridDim.x + 1)));
        CUDA_CHECK(cudaMemsetAsync(ScanValue, 0, sizeof(unsigned int) * (GridDim.x + 1)));
        CUDA_CHECK(cudaMemsetAsync(RadixSortResult, 0, sizeof(unsigned int) * (N + 1)));
        ...
        RadixSortKernel<BLOCK_DIM>
            <<<GridDim, BlockDim>>>(Input, RadixSortResult, Flags, ScanValue, Bit, N);
        ScatterValues<BLOCK_DIM><<<GridDim, BlockDim>>>(Input, RadixSortResult, Output, Bit, N);
    ...
    CUDA_CHECK(cudaFree(RadixSortResult));
...
int main()
{
    ...
    unsigned int *Lines = (unsigned int*)malloc(sizeof(unsigned int)*MAX_VALUE);
    ...
    RadixSort(DeviceLines, Output, LineIter);
