/*
 * NAME
 * 	rsortf - sorts floating point data using the gpu with radix sort
 *
 * SYNOPSIS
 * 	rsortf
 *
 * DESCRIPTION
 * 	Sorts floating point lines using radix sort to stdout.
 *
 * USAGE
 * 	Sort numbers from 1 to 1000 after shuffling
 * 	- fseq -100.0 100.0 100 | shuf | rsort
 *
 */
// #define LEET_GPU
...
__global__ void FloatToU32(unsigned int *X, int N)
{
    int Tid = blockDim.x * blockIdx.x + threadIdx.x;
    if (Tid < N)
    {
...
__global__ void U32ToFloat(unsigned int *X, int N)
{
    int Tid = blockDim.x * blockIdx.x + threadIdx.x;
    if (Tid < N)
    {
...
template <int BLOCK_DIM = 256> static void ExclusiveSum(const unsigned int *XS, unsigned int *Output, int N)
{
    ...
    FloatToU32<<<GridDim, BlockDim>>>(Input, N);
    ...
    U32ToFloat<<<GridDim, BlockDim>>>(Output, N);
...
{
    ExclusiveSum((unsigned int *)data, (unsigned int *)data, N);
}
#else
static float Lines[MAX_VALUE];
