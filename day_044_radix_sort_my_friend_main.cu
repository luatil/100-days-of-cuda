...
template <int BLOCK_DIM = 256> static void ExclusiveSum(const unsigned int *XS, unsigned int *Output, int N)
{
    ...
    // CUDA_CHECK(cudaMalloc(&Input, sizeof(int) * N )); // Have to compute number of zeros
    ...
    for (int Bit = 0; Bit < 1; Bit++)
    {
        ...
        ExclusiveSumKernel<BLOCK_DIM>
            <<<GridDim, BlockDim>>>(XS, ExclusiveSumResult, BlockCounter, Flags, ScanValue, Bit, N);
