...
void TestBenchmark(nvbench::state &State)
{
    ...
    state.exec(nvbench::exec_tag::sync,
               [&](nvbench::launch &launch) { auto Result = Convolution1D<1024 * 1024, 4>(Input, Kernel); });
}

NVBENCH_BENCH(TestBenchmark).set_timeout(10);
