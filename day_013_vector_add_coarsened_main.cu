...
#include "day_013_vector_add_coarsened_kernel.cu"
...
int main(int ArgumentCount, char **Arguments)
{
    ...
    if (ArgumentCount == 2)
    {
...

        {
            // Kernel Launch
            {
                TimeCudaBandwidth("Add Kernel Coarsened 2", 2 * SizeInBytes, SizeInBytes, 1.0f / (3.0f * 4.0f));

                int MinGridSize = 0, BlockSize = 0, GridSize = 0;

                cudaOccupancyMaxPotentialBlockSize(&MinGridSize, &BlockSize, AddKernelCoarsened2, 0, 0);
                ...
                AddKernelCoarsened2<<<GridSize, BlockSize>>>(DeviceA, DeviceB, DeviceC, N);
            }
